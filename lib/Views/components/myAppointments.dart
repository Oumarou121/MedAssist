...
class MyAppointmentsList extends StatefulWidget {
  ...
  const MyAppointmentsList({super.key, required this.managersDoctors});
...
class _MyAppointmentsListState extends State<MyAppointmentsList> {
  List<AppointmentData> appointmentDatas = [];
  ...
  Future<void> _loadAppointments() async {
    appointmentDatas = await widget.managersDoctors.getAppointments();
    setState(() {});
  }
  ...
  Widget _buildAppointmentsList(BuildContext context) {
    ...
    return SizedBox(
      ...
      child: ListView.separated(
        ...
        itemBuilder:
            (context, index) =>
                _AppointmentCard(appointmentData: todayAppointments[index]),
      ),
    );
  }

  List<AppointmentData> _getTodayAppointments({
    required List<AppointmentData> appointmentDatas,
  }) {
...
class _AppointmentCard extends StatelessWidget {
  ...
  const _AppointmentCard({required this.appointmentData});
        ...
        child: Row(
          children: [
              ...
              child: Column(
                ...
                children: [
                  Text(
                    DateFormat(
                      'HH:mm',
                    ).format(appointmentData.appointment.startTime),
                    style: GoogleFonts.poppins(
                      fontSize: 18,
                      fontWeight: FontWeight.w600,
            ...
            Padding(
              ...
              child: CircleAvatar(
                radius: 28,
                backgroundImage: NetworkImage(appointmentData.doctor.imageUrl),
